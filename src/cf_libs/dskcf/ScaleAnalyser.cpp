...
ScaleAnalyser::ScaleAnalyser(const std::vector<double> & scales, const double outputSigmaFactor, const int cellSize, double padding)
	{
		...
		for (size_t i = 1; i < this->m_scales.size(); i++)
			{
				this->m_step = std::min<double>(this->m_step, std::abs(this->m_scales[i] - this->m_scales[i - 1]));
			}
		...
		if ((boundingBox.width > 0) && (boundingBox.height > 0))
			{
				...
				for (auto itr = this->m_observers.begin(); itr != this->m_observers.end(); itr++)
					{
						//把相应的值，传递给登记进来的类
						(*itr)->onScaleChange(this->m_targetSizes[this->m_i], this->m_windowSizes[this->m_i], this->m_yfs[this->m_i], this->m_cosineWindows[this->m_i]);
					}
			}
...
Rect ScaleAnalyser::update(const cv::Mat & image, const Rect & boundingBox)
	{
		...
		if (std::abs(scaleOffset) > this->m_minStep)       //尺度变化超过需要调整的阈值
			{
				if ((scaleOffset < 0) && (this->m_i > 0)) //目标远离
					{
						size_t ind = 0;
				...
				else if ((scaleOffset > 0) && (this->m_i < this->m_scales.size())) //目标接近
					{
						size_t ind = 0;
						std::vector<double> diffs(this->m_scales.begin() + this->m_i, this->m_scales.end());
...
cv::Mat2d ScaleAnalyser::scaleImageFourier(const cv::Mat2d & image_f, const cv::Size2i & size)
	{
		if ((image_f.size().width != size.width) || (image_f.size().height != size.height))
			{
				cv::Mat result;
				cv::Rect2i inputRect, outputRect;
				cv::Size2i inputSize, outputSize;

				inputSize = image_f.size(); //输入图像的size
				outputSize = size;         //输出图像的size
...
//第一个条件 输入图像的宽是偶数，输出的宽度大于输入的宽度，图像要插值以增大
//第二个条件 输入图像的宽是奇数，输出的宽度小于输入的宽度，图像要减值以缩小
				if (((inputSize.width % 2 == 0) && (outputSize.width > inputSize.width)) || ((inputSize.width % 2 == 1) && (outputSize.width < inputSize.width)))
					{//cvCeil	进1法
						inputRect.x = cvCeil((image_f.cols - inputRect.width) / 2.0);
						outputRect.x = cvCeil((size.width - outputRect.width) / 2.0);
					}
//第一个条件 输入图像的宽是偶数，输出的宽度小于输入的宽度，图像要减值以缩小
//第二个条件 输入图像的宽是奇数，输出的宽度大于输入的宽度，图像要插值以增大
				else
					{//cvFloor 去1法
				...
				if (((inputSize.height % 2 == 0) && (outputSize.height > inputSize.height)) || ((inputSize.height % 2 == 1) && (outputSize.height < inputSize.height)))
					{
				...
				//?????????????确定没错吗？  这样 gainx和gainy 一直是1，，，没有增益
				//这个系数之后会抵消掉，，修改之后也没什么用
				double gainx = (static_cast<double>(outputRect.width) / static_cast<double>(inputRect.width));
				double gainy = (static_cast<double>(outputRect.height) / static_cast<double>(inputRect.height));
				//double gainx = (static_cast<double>(size.width) / static_cast<double>(image_f.cols));
				//double gainy = (static_cast<double>(size.height) / static_cast<double>(image_f.rows));
...
cv::Mat2d ScaleAnalyser::scaleImageFourierShift(const cv::Mat2d & image, const cv::Size2i & size)
	{
		return ifftshift(ScaleAnalyser::scaleImageFourier(fftshift(image), size));
	}
