		...
		if (_frameIdx == 0)
			{
				...
				/*				cv::Mat image_00(_image[1].rows,_image[1].cols,CV_16UC1,cv::Scalar::all(8));
				 image_00=image_00.mul(_image[1]);*/
				/*				cv::Mat image_ = _image[1].clone();
				 cv::rectangle(image_, _boundingBox.tl(), _boundingBox.br(), cv::Scalar(255));
				 cv::imshow("image_00", image_);
				 cv::waitKey(0);*/
			}
		else
			{
				...
				/*			cv::Mat image_0(_image[1].rows,_image[1].cols,CV_16UC1,cv::Scalar::all(8));
				 image_0=image_0.mul(_image[1]);*/
				/*				cv::Mat image_ = _image[1].clone();
				 cv::rectangle(image_, _boundingBox.tl(), _boundingBox.br(), cv::Scalar(255));
				 cv::imshow("image_00", image_);
				 cv::waitKey(0);*/
		...
		if (_frameIdx == 0)
			{
				...
				/*
				 if (_targetOnFrame)std::cout << "   " << _boundingBox.x << "   " << _boundingBox.y << "   " << _boundingBox.x + _boundingBox.width << "   " << _boundingBox.y + _boundingBox.height << "   " << std::endl;
				 */
			}
		else if (_frameIdx >= 1)
			{
				...
				//cv::Mat image_2(resized[1].rows, resized[1].cols, CV_16UC1, cv::Scalar::all(8));
				//image_2 = image_2.mul(resized[1]);
				...
				/*
				 ...
				 else std::cout << "   NaN   NaN   NaN   NaN   " << 1 << std::endl;
				 */
